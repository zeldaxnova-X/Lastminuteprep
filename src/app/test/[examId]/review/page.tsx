...
export default function ReviewAnswersPage() {
  ...
  if (loading) {
    return (
      <div className="h-screen w-screen flex flex-col items-center justify-center bg-white text-gray-700 gap-3 font-sans">
        ...
        <span className="text-sm font-bold tracking-wide uppercase">Loading Solutions from Single Source of Truth Engine...</span>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 font-sans flex flex-col antialiased select-none">
      ...
      {/* Main Review Content */}
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 flex-1 space-y-6">
        {/* Controls Bar */}
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-2xs space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="text-xs bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-200 text-gray-700 font-medium flex items-center gap-2">
              <span>Showing:</span>
        ...
        {filteredQuestions.length === 0 ? (
          <div className="text-center py-16 bg-white border border-gray-200 rounded-xl">
            <p className="text-gray-500 font-medium">No questions match your selected filter.</p>
            <button
              onClick={() => setFilter("ALL")}
          ...
          <div className="space-y-6">
            {filteredQuestions.map((q) => {
              ...
              return (
                <div
                  key={q.id}
                  className="bg-white border border-gray-200 rounded-xl p-6 shadow-2xs space-y-5"
                >
                  {/* Card Header */}
                  <div className="flex flex-wrap items-center justify-between border-b border-gray-100 pb-3 gap-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="bg-gray-100 text-gray-700 font-semibold px-2.5 py-0.5 rounded text-xs border border-gray-200">
                        Q{q.questionNumber} • {q.section}
                      </span>

                      {/* Time Spent Badge */}
                      <span className="bg-gray-50 border border-gray-200 text-gray-600 px-2 py-0.5 rounded text-xs font-mono flex items-center gap-1">
                      ...
                      {/* Rapid Guess Badge */}
                      {isRapidGuess && (
                        <span className="bg-amber-50 border border-amber-200 text-amber-700 px-2 py-0.5 rounded text-xs font-semibold flex items-center gap-1">
                          <ZapOff className="w-3 h-3 text-amber-500" />
                          <span>Rapid Guess (&le;8s)</span>
                        </span>
                      )}

                      {/* Answer Changed Badge */}
                      {changes > 0 && (
                        <span className="bg-blue-50 border border-blue-200 text-blue-700 px-2 py-0.5 rounded text-xs font-semibold flex items-center gap-1">
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg bg-white border border-gray-200 text-gray-500 hover:text-red-600 text-xs font-medium transition-colors flex items-center gap-1"
                        title="Report Issue"
                      >
                    ...
                    <div>
                      {isCorrect && (
                        <span className="bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-semibold px-2.5 py-0.5 rounded-full flex items-center gap-1">
                          <CheckCircle2 className="w-3.5 h-3.5" /> Correct (+{q.positiveMarks.toFixed(1)})
                        </span>
                      )}
                      {!isCorrect && !isUnattempted && (
                        <span className="bg-red-50 border border-red-200 text-red-600 text-xs font-semibold px-2.5 py-0.5 rounded-full flex items-center gap-1">
                          <XCircle className="w-3.5 h-3.5" /> Incorrect (-{q.negativeMarks.toFixed(2)})
                        </span>
                      )}
                      {isUnattempted && (
                        <span className="bg-gray-100 text-gray-500 text-xs font-semibold px-2.5 py-0.5 rounded-full flex items-center gap-1">
                  ...
                  <div className="bg-blue-50/70 border border-blue-200 rounded-xl p-3 flex items-start gap-2 text-xs">
                    ...
                    <span className="text-gray-800 font-medium">{mentorNote}</span>
                  </div>

                  {/* Question Text (Sanitized) */}
                  <div className="text-base text-gray-900 font-medium leading-relaxed bg-gray-50 border border-gray-200 rounded-xl p-5">
                  ...
                  <div className="space-y-2">
                    ...
                    {q.options.map((option) => {
                      ...
                      if (isUserPicked && !isOfficialCorrect) {
                        ...
                        badge = (
                          <span className="bg-red-600 text-white text-[11px] font-semibold px-2 py-0.5 rounded flex items-center gap-1">
                        ...
                        badge = (
                          <span className="bg-emerald-600 text-white text-[11px] font-semibold px-2 py-0.5 rounded flex items-center gap-1">
                      ...
                      return (
                        <div
                          key={option.id}
                          className={`p-3.5 rounded-xl border transition-all flex items-center justify-between gap-3 ${containerStyles}`}
                        >
                          <div className="flex items-center gap-3">
                            <div className="w-6 h-6 rounded-full bg-gray-100 border border-gray-300 text-gray-600 font-bold text-xs flex items-center justify-center flex-shrink-0">
                              {option.id}
                            </div>
                  ...
                  <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-4 space-y-2">
                    ...
                    <div className="text-sm text-gray-800 leading-relaxed pt-1">
              ...
              <>
                ...
                <div className="flex gap-2 pt-1">
                  <button
                    ...
                    className="flex-1 py-2 rounded-lg border border-gray-200 text-gray-600 font-semibold text-xs hover:bg-gray-50"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={handleReportSubmit}
                    className="flex-1 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs shadow-xs"
                  >
                    Submit Report
                  </button>
                </div>
